...
//        menus.forEach(System.out::println);
        ...
        Order order = new Order(orderInfo.getOrderDate(), orderInfo.getOrderTime()
                , totalOrderPrice);

        /* 설명. 4-2 Order로 tbl_order 테이블에 insert */
        orderMapper.insertOrder(order); //select Key를 활용해서 insert 이후 seelct을 통해 한번의 DB와의 I/O를 발생시킴
        System.out.println("주문 성공 후 방금 주문한 번호가 Order Entity에 들어있는지 확인: " + order.getOrderCode());
